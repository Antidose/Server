...
func startIncidentHandler(w http.ResponseWriter, r *http.Request) {
    ...
    incID := randString(12)
    queryString = "INSERT INTO incidents(inc_id, requester_imei, init_req_location, time_start) " +
    ...
    for userID := range responderCandidates {
        ...
        queryString =   "SELECT ST_X(help_location), ST_Y(help_location), firebase_id " +
                        "FROM users NATURAL JOIN location WHERE u_id = $1"
        stmt, err = db.Prepare(queryString)
        ...
        queryString =   "INSERT INTO requests(u_id, init_time, inc_id, init_help_location) " +
